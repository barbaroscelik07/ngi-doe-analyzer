    ...
    def _fill_anova(self, anova):
        ...
        for ri, (idx, row) in enumerate(anova.iterrows()):
            ...
            for ci, col in enumerate(cols):
                ...
                # mean_sq hesapla (ANOVA'da bazen eksik gelir)
                if col == "mean_sq":
                    df_val = row.get("df", np.nan)
                    ss_val = row.get("sum_sq", np.nan)
                    if (isinstance(df_val, float) and not math.isnan(df_val)
                            and isinstance(ss_val, float) and not math.isnan(ss_val)
                            and df_val > 0):
                        val = ss_val / df_val
                    else:
                        val = np.nan
